...
const totalMarkCalculate = (studentsObject) => {
    const totalMark = studentsObject.map((student) => student.marks.reduce((acc, mark) => acc + mark, 0));
    return totalMark;
};
// Average Marks
const calculateAverageMark = (studentsObject) => {
    const averageMark = studentsObject.map((student) => {
        let average = student.marks.reduce((acc, mark) => acc + mark, 0) / student.marks.length;
...
const calculateGrade = (averageMark) => {
    const grade = averageMark.map((mark) => {
        if (mark >= 80) {
            return "A+";
        }
        else if (mark >= 70) {
